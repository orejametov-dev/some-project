...
namespace App\Http\Controllers\ApiMerchantGateway\AlifshopMerchant;
...
class AlifshopMerchantAccesses extends ApiBaseController
{
    public function index(Request $request)
    {
        $merchantUsersQuery = AlifshopMerchantAccess::query()
            ->with(
                'company_user:id,user_id,phone,full_name',
                'alifshop_merchant',
                'store'
            )
            ->byAlifshopMerchant($this->alifshop_merchant_id)
    ...
    {
        $merchantUser = AlifshopMerchantAccess::query()
            ->with(
                'company_user:id,user_id,phone,full_name',
                'alifshop_merchant',
                'store'
    ...
    public function update($id, Request $request)
    {
        ...
        $alifshop_merchant_store = $alifshop_merchant->stores()->where(['id' => $request->input('store_id')])->firstOrFail();
        ...
        SendHook::dispatch(new HookData(
            ...
            keyword: 'old_store: (' . $old_store->id . ', ' . $old_store->name .
            ') -> ' . 'store: (' . $alifshop_merchant_store->id . ', ' . $alifshop_merchant_access->name . ')',
            action: 'update',
            class: 'warning',
            action_at: null,
    ...
    public function store(Request $request)
    {
        ...
        if ($alifshop_merchant_access = AlifshopMerchantAccess::withTrashed()->where('company_user_id', $company_user->id)->first()) {
        ...
        ToggleMerchantRoleOfUser::dispatch($alifshop_merchant_access->company_user_id, AuthMicroService::ACTIVATE_MERCHANT_ROLE);

        Cache::tags('azo_merchants')->forget('azo_merchant_user_id_' . $alifshop_merchant_access->company_user_id);
        Cache::tags($alifshop_merchant->id)->flush();
    ...
    public function destroy($id)
    {
        ...
        Cache::tags('alifshop_merchants')->forget('azo_merchant_user_id_' . $alifshop_merchant_access->company_user_id);
        Cache::tags($alifshop_merchant->id)->flush();
