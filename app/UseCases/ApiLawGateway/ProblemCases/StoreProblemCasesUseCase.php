...
namespace App\UseCases\ApiLawGateway\ProblemCases;
...
class StoreProblemCasesUseCase
{
    ...
    {
        $data = $this->coreHttpRepository->getApplicationDataByContractNumber($problemCaseDTO->credit_number);

        if (ProblemCase::query()->where('credit_number', $problemCaseDTO->credit_number)
        ...
        $problemCase->credit_number = $problemCaseDTO->credit_number;
